
import { useState } from "react";
import { allOrders } from "../../../data/mockData.js";
import OrderCard      from "../../../components/UI/OrderCard/OrderCard.jsx";
import FilterDropdown from "../../../components/UI/FilterDropdown/FilterDropdown.jsx";
import AddOrderModal  from "../../../components/UI/AddOrderModal/AddOrderModal.jsx";
import "./Orders.css";

/* ── Status tabs ──────────────────────────────────────────────────────────
   Order matches the design: All → Assigned → In Progress → Completed → Delayed
─────────────────────────────────────────────────────────────────────────── */
const TABS = [
  { id: "All",         label: "All"         },
  { id: "Assigned",    label: "Assigned"    },
  { id: "In Progress", label: "In Progress" },
  { id: "Completed",   label: "Completed"   },
  { id: "Delayed",     label: "Delayed"     },
];

/* ── Sort comparators — keyed to FilterDropdown's SORT_OPTIONS ids ────────
─────────────────────────────────────────────────────────────────────────── */
const SORTERS = {
  "delivery-asc":  (a, b) => new Date(a.deliveryDate) - new Date(b.deliveryDate),
  "delivery-desc": (a, b) => new Date(b.deliveryDate) - new Date(a.deliveryDate),
  "client-asc":    (a, b) => a.client.localeCompare(b.client),
  "client-desc":   (a, b) => b.client.localeCompare(a.client),
  "id-asc":        (a, b) => a.id.localeCompare(b.id),
  "id-desc":       (a, b) => b.id.localeCompare(a.id),
};

/* ══════════════════════════════════════════════════════════════════════════
   ORDERS PAGE COMPONENT
══════════════════════════════════════════════════════════════════════════ */
export default function Orders() {
  /* ── State ──────────────────────────────────────────────────────────── */
  const [orders, setOrders]       = useState(allOrders); // swap → fetch('/api/orders')
  const [query,  setQuery]        = useState("");
  const [activeTab, setActiveTab] = useState("All");
  const [activeSort, setActiveSort]     = useState(null);   // null = default order
  const [modalOpen, setModalOpen]       = useState(false);

  /* ── Filter pipeline ────────────────────────────────────────────────────
     Step 1 — search query  (client name, order ID, description)
     Step 2 — active status tab
     Step 3 — sort
  ────────────────────────────────────────────────────────────────────── */
  const q = query.toLowerCase();

  const filtered = orders
    .filter((o) =>
      !q ||
      o.client.toLowerCase().includes(q) ||
      o.id.toLowerCase().includes(q) ||
      o.description.toLowerCase().includes(q)
    )
    .filter((o) => activeTab === "All" || o.status === activeTab)
    .sort(activeSort ? SORTERS[activeSort] : () => 0);

  /* ── Tab counts — always based on full list, not filtered ───────────── */
  const countFor = (tabId) =>
    tabId === "All"
      ? orders.length
      : orders.filter((o) => o.status === tabId).length;

  /* ── Handlers ────────────────────────────────────────────────────────── */
  const handleView = (order) => {
    // TODO: navigate to /orders/:id detail page
    console.log("View order:", order.id);
  };

  const handleOrderCreated = (newOrder) => {
    // Optimistic UI — prepend to list; replace with real API response later
    const id = `ORD-${String(orders.length + 1).padStart(3, "0")}`;
    setOrders((prev) => [
      {
        ...newOrder,
        id,
        delivery: new Date(newOrder.deliveryDate).toLocaleDateString("en-US", {
          month: "short", day: "numeric",
        }),
        image: "https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=400&h=220&fit=crop&q=80",
      },
      ...prev,
    ]);
  };

  /* ══════════════════════════════════════════════════════════════════════
     RENDER
  ════════════════════════════════════════════════════════════════════════ */
  return (
    <div className="op">

      {/* ── Toolbar ──────────────────────────────────────────────────────
          Search | Filter dropdown | Add Order button
      ─────────────────────────────────────────────────────────────────── */}
      <div className="op__toolbar">

        {/* Search */}
        <div className="op__search-wrap">
          {/* Inline search SVG — explicit 16x16 */}
          <svg className="op__search-icon" viewBox="0 0 24 24" fill="none"
            stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <circle cx="11" cy="11" r="8"/>
            <line x1="21" y1="21" x2="16.65" y2="16.65"/>
          </svg>
          <input
            className="op__search-input"
            type="text"
            placeholder="Search Orders"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            aria-label="Search orders"
          />
          {/* Clear search */}
          {query && (
            <button
              className="op__search-clear"
              onClick={() => setQuery("")}
              aria-label="Clear search"
              type="button"
            >
              {/* Explicit 14x14 */}
              <svg className="op__search-clear-icon" viewBox="0 0 24 24" fill="none"
                stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
                <line x1="18" y1="6"  x2="6"  y2="18"/>
                <line x1="6"  y1="6"  x2="18" y2="18"/>
              </svg>
            </button>
          )}
        </div>

        {/* Actions */}
        <div className="op__actions">
          <FilterDropdown
            activeSort={activeSort}
            onSortChange={setActiveSort}
          />
          <button
            className="op__add-btn"
            onClick={() => setModalOpen(true)}
            type="button"
          >
            {/* Plus icon — explicit 16x16 */}
            <svg className="op__add-icon" viewBox="0 0 24 24" fill="none"
              stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
              <line x1="12" y1="5" x2="12" y2="19"/>
              <line x1="5"  y1="12" x2="19" y2="12"/>
            </svg>
            Add Order
          </button>
        </div>
      </div>

      {/* ── Status tabs ──────────────────────────────────────────────────
          All | Assigned | In Progress | Completed | Delayed
      ─────────────────────────────────────────────────────────────────── */}
      <div className="op__tabs" role="tablist" aria-label="Filter orders by status">
        {TABS.map((tab) => {
          const count = countFor(tab.id);
          return (
            <button
              key={tab.id}
              role="tab"
              aria-selected={activeTab === tab.id}
              className={`op__tab ${activeTab === tab.id ? "op__tab--active" : ""}`}
              onClick={() => setActiveTab(tab.id)}
            >
              {tab.label}
              {tab.id !== "All" && count > 0 && (
                <span className={`op__tab-count op__tab-count--${tab.id.toLowerCase().replace(" ", "")}`}>
                  {count}
                </span>
              )}
            </button>
          );
        })}
      </div>

      {/* ── Results summary line ──────────────────────────────────────── */}
      {(query || activeSort) && (
        <p className="op__results-meta">
          {filtered.length} order{filtered.length !== 1 ? "s" : ""} found
          {activeSort && <span className="op__results-sort"> · sorted</span>}
        </p>
      )}

      {/* ── Card grid / empty state ───────────────────────────────────── */}
      {filtered.length === 0 ? (
        <div className="op__empty">
          <span className="op__empty-icon">🔍</span>
          <p className="op__empty-text">
            {query
              ? `No orders match "${query}"`
              : `No ${activeTab !== "All" ? activeTab : ""} orders yet`}
          </p>
          {query && (
            <button className="op__empty-clear" onClick={() => setQuery("")} type="button">
              Clear search
            </button>
          )}
        </div>
      ) : (
        <div className="op__grid">
          {filtered.map((order) => (
            <OrderCard
              key={order.id}
              order={order}
              onView={handleView}
            />
          ))}
        </div>
      )}

      {/* ── Add Order Modal ───────────────────────────────────────────── */}
      <AddOrderModal
        isOpen={modalOpen}
        onClose={() => setModalOpen(false)}
        onSubmit={handleOrderCreated}
      />

    </div>
  );


}
